...
function createTable()  {
    ...
    var createSQL = 'create table payment(id integer primary key autoincrement ,name text, category text, year integer, month integer, day integer, amount integer, plus boolean';
...
function insertPayment(){
    db.transaction(function(tr){
        ...
        var plus;
        if($('#PaymentInfo').val()==='수입')
            plus=true;
        else
            plus=false;
        var insertSQL = 'insert into payment(name,category,year,month,day,amount,plus) values(?,?,?,?,?,?,?,?)';
        tr.excuteSql(insertSQL,[name,category,year,month,day,amount,plus], function(tr,rs){
...
function updatePayment(){
    db.transaction(function(tr){
        ...
        var updateSQL = 'update payment set name=? category=? amount=? plus=? where id=?';
        tr.executeSql(updateSQL,[newName,newCategory,newAmount,newPlus,paymentID],function(tr,rs){
            console.log('내역 수정');
